...
import { convertUnicodeToDevlys, convertDevlysToUnicode } from '../../utils/devlysConverter';
...
const ENGLISH_PRESET_PARAGRAPH =
  'The District Administration of Rajsamand is committed to ensuring prompt, transparent, and accountable delivery of public services to all citizens. Under the visionary governance model of the Government of Rajasthan, various administrative reforms and e-governance initiatives have been successfully implemented across all seven tehsils. The objective of this official speed assessment is to evaluate the computer typing proficiency, accuracy, and operational agility of candidate personnel. Efficient document drafting and rapid data entry play a vital role in government offices, particularly in citizen-centric portals such as e-Mitra, Jan Soochna Portal, and Rajasthan Sampark grievance redressal portal. All participating candidates must maintain consistent rhythm and strict adherence to punctuation throughout the allotted duration of ten minutes.';
...
export const CreateTypingTestModal: React.FC<CreateTypingTestModalProps> = ({
  ...
  const handleSaveWithStatus = (submitStatus: 'PUBLISHED' | 'DRAFT' | 'REVOKED') => {
    if (!title.trim()) {
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white px-6 py-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl bg-amber-500/10 dark:bg-amber-600/20 border border-amber-500/30 flex items-center justify-center text-amber-700 dark:text-amber-400 font-extrabold text-lg">
        ...
        >
              ...
              <input
                ...
                placeholder={
                  language === 'HINDI_DEVLYS_010'
                    ...
                    : 'e.g., District Speed Assessment (Set 1)'
                }
          ...
          <div>
            ...
            {passageText.trim().length > 0 && (
              <div className="mt-2.5 p-2.5 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/60 flex flex-wrap items-center justify-between gap-2 text-xs text-emerald-800 dark:text-emerald-300">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                  <span className="font-bold">
                    {language === 'HINDI_DEVLYS_010'
                      ? 'DevLys 010 Passage Ready (100% Clean & Validated)'
                      : 'English Passage Ready (100% Clean & Validated)'}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => {
                      // Silently collapse repeated spaces and trim
                      const cleaned = passageText
                        .replace(/\r\n/g, '\n')
                        .replace(/[\u200B-\u200D\uFEFF]/g, '')
                        .replace(/[ \t]+/g, ' ')
                        .trim();
                      setPassageText(cleaned);
                    }}
                    className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-750 text-[11px] font-bold flex items-center gap-1 shadow-2xs transition-all cursor-pointer"
                    title="Clean extra spaces between words"
                  >
