...
import { Candidate, TestPaper, TestAttempt } from '../types';
import { Compass, BarChart3, Clock, FileCheck, Award, ArrowRight, CheckCircle2, AlertTriangle, Mail, ShieldCheck, Play, Key, Download } from 'lucide-react';
...
export const CandidatePortal: React.FC<CandidatePortalProps> = ({
  ...
  const avgScore =
    completedCount > 0
      ? Math.round(myAttempts.reduce((sum, a) => sum + a.scorePercentage, 0) / completedCount)
      : 0;

  const passedCount = myAttempts.filter((a) => a.status === 'PASSED').length;
  ...
  const availableTests = tests.filter((t) => {
    ...
    // Check block match
    const matchesBlock = t.targetBlock === 'District-Wide' || t.targetBlock === candidate.block;
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-gradient-to-r from-emerald-900 via-slate-900 to-teal-950 p-6 sm:p-8 rounded-2xl text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          <div className="w-16 h-16 rounded-2xl bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 font-bold text-2xl flex items-center justify-center flex-shrink-0 shadow-inner">
          ...
          <div>
            ...
            <p className="text-xs text-slate-300 mt-1">
              Assigned Block: <strong className="text-white">{candidate.block} Tehsil</strong> • Category: {candidate.category}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 bg-white/5 backdrop-blur-md p-4 rounded-xl border border-white/10 text-center text-xs">
          <div>
            <span className="text-slate-400 block text-[10px] uppercase">Completed</span>
          ...
          <div>
            ...
            <span className="text-lg font-bold text-emerald-400">{avgScore}%</span>
          </div>
          <div>
            <span className="text-slate-400 block text-[10px] uppercase">Qualified</span>
            <span className="text-lg font-bold text-sky-400">{passedCount}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 dark:border-slate-800 pb-2">
        ...
        >
          <BarChart3 className="w-4 h-4" /> Real-time Performance ({completedCount})
        </button>
      </div>

      {/* TAB 1: Available Assessments List */}
      {activeTab === 'my-tests' && (
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-3 text-xs text-slate-600 dark:text-slate-300 font-medium">
                      ...
                      <span>• Target: {test.targetBlock}</span>
                    </div>
                  </div>
                    ...
                    {existingAttempt ? (
                      <div className="flex items-center justify-between w-full">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                        >
                          Review Answers
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-md transition-all flex items-center justify-center space-x-2"
                      >
      ...
      {activeTab === 'my-performance' && (
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">My Real-Time Assessment History</h3>
                      ...
                      <tr key={att.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="px-5 py-4 text-right space-x-2">
                          <button
                            ...
                            className="p-1.5 rounded-lg text-emerald-700 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 text-[11px] font-semibold inline-flex items-center gap-1"
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg text-sky-600 dark:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-950/50 text-[11px] font-semibold inline-flex items-center gap-1"
                          >
